...
#classesFile = 'app\obj.names';
classes = "helmet"
# Give the configuration and weight files for the model and load the network using them.
modelConfiguration = 'app\yolov3-obj.cfg'
modelWeights = 'app\yolov3-obj_2400.weights'

net = cv.dnn.readNetFromDarknet('app/yolov3.txt', 'app/yolov3.weights')
...
def helmetcounter(input_image):
    ...
    #cv.namedWindow(winName, cv.WINDOW_NORMAL)
        ...
        blob = cv.dnn.blobFromImage(input_image, 1/255, (inpWidth, inpHeight), [0,0,0], 1, crop=False)
        ...
        count=postprocess(input_image, outs)

        # Put efficiency information. The function getPerfProfile returns the overall time for inference(t) and the timings for each of the layers(in layersTimes)
        t, _ = net.getPerfProfile()
        #print(t)
        infrence_time = t * 1000.0 / cv.getTickFrequency()
        #print(label)
        #cv.putText(Input_image, label, (0, 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
        #print(label)
        return count,infrence_time
